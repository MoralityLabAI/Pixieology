"""Fail-closed, job-bound authorization receipts for capture v0.2."""
...
AUTHORIZATION_SCHEMA = "pixie_etale_capture_authorization_v2"
...
def validate_authorization(
    ...
    *,
    ...
    if value.get("schema") != AUTHORIZATION_SCHEMA:
        raise ValueError("invalid capture v0.2 authorization schema")
    if value.get("authorized") is not True:
        raise ValueError("capture v0.2 authorization is not active")
    ...
    if require_active_wrapper:
        if os.environ.get("PIXIE_RESOURCE_CAP_ACTIVE") != "1":
            raise ValueError("capture v0.2 must run inside the resource-cap wrapper")
