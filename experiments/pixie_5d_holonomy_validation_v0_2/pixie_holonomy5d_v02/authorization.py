"""Exact authorization gate for the v0.2 GPU continuation."""
...
SCHEMA = "pixie_5d_context3_authorization_v2"
STATEMENT = "I explicitly authorize the v0.2 context-3 continuation under the exact caps in this receipt."
...
def validate(
    ...
    *,
    ...
    if value.get("authorization_statement") != STATEMENT:
        raise AuthorizationError("explicit authorization statement is missing or changed")
    ...
    if require_active_wrapper:
        ...
        if mismatch:
            raise AuthorizationError(f"continuation is outside the exact v0.2 wrapper: {mismatch}")
    return value
