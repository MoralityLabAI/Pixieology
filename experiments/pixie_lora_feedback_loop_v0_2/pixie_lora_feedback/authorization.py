...
def authorization_template(
    ...
    return {
        ...
        "acknowledgements": {
            ...
            "spot_interruption_is_valid_abort": False,
...
def validate_authorization(
    ...
    *,
    ...
    required = {
        ...
        "pid_scoped_cleanup",
        "paid_primelab_pod",
        "spot_interruption_is_valid_abort",
    ...
    if require_active_wrapper:
        ...
        expected_environment = {
            "PIXIE_CAP_RAM_MB": str(value["caps"]["ram_mb"]),
            "PIXIE_CAP_CPU_PCT": str(value["caps"]["cpu_pct"]),
            "PIXIE_CAP_IO_MB_S": str(value["caps"]["io_mb_s"]),
            "PIXIE_CAP_TIMEOUT_SECONDS": str(value["caps"]["timeout_seconds"]),
        }
        for name, expected in expected_environment.items():
            if os.environ.get(name) != expected:
                raise ValueError(f"wrapper environment {name} differs from feedback authorization")
