...
def _runtime_path(
    ...
    return Path(override).expanduser().resolve() if override else resolve_config_path(repo_root, config, key)
...
def verify(
    ...
    *,
    ...
    continuation_root = experiment_root.parent / str(protocol["continuation_of"]["experiment_id"])
    model_root = _runtime_path(
        repo_root,
        config,
        "godel_globes_bonsai_unpacked_hf",
        "PIXIE_FEEDBACK_MODEL_ROOT",
    )
    adapter_root = _runtime_path(
        repo_root,
        config,
        "godel_globes_bonsai_adapter",
        "PIXIE_FEEDBACK_ADAPTER_ROOT",
    )
    ...
    checks: dict[str, Any] = {
        ...
        "continuation_protocol": (continuation_root / "protocol.json").is_file()
        and sha256_file(continuation_root / "protocol.json") == protocol["continuation_of"]["protocol_sha256"],
        "continuation_lock": (continuation_root / "protocol.lock.json").is_file()
        and sha256_file(continuation_root / "protocol.lock.json")
        == protocol["continuation_of"]["implementation_lock_sha256"],
        "primelab_launcher": (experiment_root / protocol["bounded_launcher"]["primelab_entrypoint"]).is_file()
        and sha256_file(experiment_root / protocol["bounded_launcher"]["primelab_entrypoint"])
    ...
    if require_model_weights:
        ...
        if weights.is_file():
            checks["model_weights"] = sha256_file(weights) == protocol["base_model"]["weights_sha256"]
        else:
            try:
                verify_snapshot(
                    model_root,
