# Preregistration — PrimeLab resource continuation v0.2

V0.1 is sealed as a valid 2048 MiB memory-guard abort with zero registered
transfer rows. This continuation was fixed before viewing any registered base
or Pixie transfer outcome. It changes the resource envelope and execution
surface only.
...
Every executable job is bound to 8192 MiB RAM, 50% CPU, 50 MiB/s I/O, 1800
seconds, an idle-GPU preflight, and a 3900 MiB peak reserved-VRAM guard.
PrimeLab execution uses cgroup v2 before the Python process starts. Evaluation
checkpoints every completed transfer row; training checkpoints every 5
optimizer steps or 300 seconds, with at most two retained. Wrapper abort, spot
interruption, CUDA guard failure, cleanup failure, or inability to load inside
the RAM cap is a terminal attempt result and must be reported, not silently
retried with larger resources.
