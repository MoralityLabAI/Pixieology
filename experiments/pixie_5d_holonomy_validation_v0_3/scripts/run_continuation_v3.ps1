...
if ($LASTEXITCODE -ne 0) { throw "v0.3 authorization validation failed." }
...
$cleanupScript = [IO.Path]::GetFullPath((Join-Path $experimentRoot $protocol.bounded_launcher.cleanup_path))
...
$cleanupHash = (Get-FileHash -LiteralPath $cleanupScript -Algorithm SHA256).Hash.ToLowerInvariant()
if ($wrapperHash -ne $protocol.bounded_launcher.sha256 -or
    $invokerHash -ne $protocol.bounded_launcher.owned_process_gate_sha256 -or
    $cleanupHash -ne $protocol.bounded_launcher.cleanup_sha256) {
  throw "v0.3 wrapper, invoker, or cleanup script differs from the frozen protocol."
}
$outputRoot = (& $PythonExecutable $runner output-root).Trim()
$shardedRoot = (& $PythonExecutable $runner sharded-root).Trim()
if ($LASTEXITCODE -ne 0 -or -not $outputRoot -or -not $shardedRoot) {
  throw "Could not resolve configured v0.3 paths."
}
$shardDrive = Get-PSDrive -Name ([IO.Path]::GetPathRoot($shardedRoot).Substring(0, 1))
if ($shardDrive.Free -lt 5GB) { throw "Sharded-model drive has less than 5 GiB free." }
