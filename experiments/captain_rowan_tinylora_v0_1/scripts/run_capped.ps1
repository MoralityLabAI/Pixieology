...
if ($LASTEXITCODE -ne 0) { throw "Captain Rowan frozen-input verification failed." }
& $PythonExecutable $runner authorization-check --job $jobPath --authorization $authorizationPath
if ($LASTEXITCODE -ne 0) { throw "Captain Rowan exact-job authorization validation failed." }

$job = Get-Content -LiteralPath $jobPath -Raw -Encoding UTF8 | ConvertFrom-Json
$receipt = Get-Content -LiteralPath $authorizationPath -Raw -Encoding UTF8 | ConvertFrom-Json
if ((Get-FileHash -LiteralPath $runner -Algorithm SHA256).Hash.ToLowerInvariant() -ne $job.execution.continuation_runner_sha256) {
  throw "Captain Rowan continuation runner differs from the bound job."
}
if ((Get-FileHash -LiteralPath $PSCommandPath -Algorithm SHA256).Hash.ToLowerInvariant() -ne $job.execution.continuation_wrapper_sha256) {
  throw "Captain Rowan continuation wrapper differs from the bound job."
}
...
$cleanupExit = 125
try {
  & $wrapper -Executable $PythonExecutable -ArgumentsBase64 $encoded -RunId $receipt.run_id -AttemptId $receipt.attempt_id -OutputDirectory $wrapperRoot -MemoryMB 8192 -CpuPercent 50 -IoMBPerSecond 50 -TimeoutSeconds 1800
