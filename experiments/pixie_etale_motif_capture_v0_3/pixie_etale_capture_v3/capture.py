"""Memory-optimized, resumable capture for the frozen five-family continuation."""
...
class CaptureV3Error(RuntimeError):
    """The v0.3 continuation cannot proceed without violating its contract."""
...
def _require_gpu_guard(torch: Any, maximum_mib: int) -> None:
    ...
    if peak > maximum_mib:
        raise CaptureV3Error(f"peak reserved VRAM {peak} MiB exceeds frozen {maximum_mib} MiB guard")
...
def _select_continuation(
    ...
    if len(selected) != row_count:
        raise CaptureV3Error("frozen continuation rows are incomplete")
    ...
    for offset in range(0, row_count, chunk_rows):
        ...
        if len(family_values) != 1:
            raise CaptureV3Error("a frozen continuation chunk mixes families")
        observed_families.append(next(iter(family_values)))
    if observed_families != job["families"]:
        raise CaptureV3Error("frozen continuation families differ from the job")
    observed_splits = {
        split: sum(row["split"] == split for row in selected)
        for split in ("discovery", "confirmation", "transfer")
    }
    if observed_splits != job["splits"]:
        raise CaptureV3Error("frozen continuation split counts differ from the job")
    return selected
...
def _checkpoint_is_resumable(
    ...
    *,
    ...
    try:
        ...
        return False
    return (
        marker.get("schema") == "pixieology_etale_capture_checkpoint_v3"
        and marker.get("capture_protocol_sha256") == capture_protocol_sha256
        and marker.get("job_sha256") == job_sha256
        and marker.get("row_ids") == row_ids
        and marker.get("artifact_sha256") == sha256_file(artifact_path)
    )
...
def capture_continuation(
    ...
    if not frozen["ok"]:
        raise CaptureV3Error(f"frozen v0.3 inputs failed: {frozen['checks']}")
    ...
    selected = _select_continuation(rows, job, chunk_rows)

    run_root = output_root / "capture_v0_3" / authorization.run_id / "chunks_01_05"
    ...
    try:
        ...
        if not torch.cuda.is_available():
            raise CaptureV3Error("CUDA is unavailable inside capped capture v0.3")
        ...
        for local_start in range(0, len(selected), checkpoint_rows):
            ...
            group_row_ids = [row["id"] for row in group]
            ...
            for row in group:
                ...
                if set(active["inputs"]) != set(modules):
                    raise CaptureV3Error(f"module hooks were incomplete for {row['id']}")
            ...
            marker = {
                ...
                "capture_protocol_sha256": protocol_hash(experiment_root),
                "job_sha256": job["authorization"]["job_sha256"],
                "chunk_index": (start + local_start) // chunk_rows,
        ...
        summary = {
            "schema": "pixieology_etale_capture_continuation_summary_v3",
            "status": "COMPLETE",
            "capture_protocol_sha256": protocol_hash(experiment_root),
            "source_protocol_sha256": protocol["source_experiment"]["protocol_sha256"],
            "job_id": job["job_id"],
    ...
    if failure is not None:
        abort = {
            "schema": "pixieology_etale_capture_abort_v3",
            "status": "ABORTED",
            "capture_protocol_sha256": protocol_hash(experiment_root),
            "source_protocol_sha256": protocol["source_experiment"]["protocol_sha256"],
            "job_id": job["job_id"],
        ...
        raise CaptureV3Error(f"capture v0.3 aborted with receipt {run_root / 'abort.json'}") from failure
    ...
    if summary["status"] != "COMPLETE":
        raise CaptureV3Error("capture v0.3 completed but cleanup failed")
    return summary
...
def finalize_execution(
    ...
    capture_root = output_root / "capture_v0_3" / run_id / "chunks_01_05"
    ...
    receipt = {
        "schema": "pixie_etale_capture_execution_summary_v3",
        "status": status,
        "job_id": job["job_id"],
        "job_sha256": job["authorization"]["job_sha256"],
        "run_id": run_id,
