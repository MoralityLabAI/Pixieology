"""Fail-closed, job-bound authorization receipts for capture v0.3."""
...
AUTHORIZATION_SCHEMA = "pixie_etale_capture_authorization_v3"
...
def authorization_template(
    ...
    return {
        ...
        "acknowledgements": {
            "model_load": False,
            "sequential_safetensors_loader": False,
            "activation_capture": False,
            "frozen_continuation_rows": False,
...
def validate_authorization(
    ...
    *,
    ...
    if value.get("schema") != AUTHORIZATION_SCHEMA:
        raise ValueError("invalid capture v0.3 authorization schema")
    if value.get("authorized") is not True:
        raise ValueError("capture v0.3 authorization is not active")
    ...
    if value.get("job_id") != job["job_id"] or value.get("job_sha256") != job_sha256(job):
        raise ValueError("authorization is bound to another capture job")
    ...
    required = (
        "model_load",
        "sequential_safetensors_loader",
        "activation_capture",
        "frozen_continuation_rows",
    ...
    if require_active_wrapper:
        if os.environ.get("PIXIE_RESOURCE_CAP_ACTIVE") != "1":
            raise ValueError("capture v0.3 must run inside the resource-cap wrapper")
