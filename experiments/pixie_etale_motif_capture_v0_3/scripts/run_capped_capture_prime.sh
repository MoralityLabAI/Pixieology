...
usage() {
  echo "usage: $0 <authorization.json> <runtime-repo-root> <sharded-model-root> <output-root>" >&2
  exit 64
}

[[ $# -eq 4 ]] || usage
AUTHORIZATION="$(realpath "$1")"
RUNTIME_REPO_ROOT="$(realpath "$2")"
SHARDED_MODEL_ROOT="$(realpath "$3")"
OUTPUT_ROOT="$(realpath -m "$4")"
[[ -f "$RUNTIME_REPO_ROOT/pixieology.config.json" ]] || {
  echo "runtime repo root lacks pixieology.config.json" >&2
  exit 65
}
...
[[ "$RAM_MB" == "6144" && "$CPU_PCT" == "50" && "$IO_MB_S" == "250" && "$TIMEOUT_SECONDS" == "1800" ]] || {
  echo "authorization caps differ from the sealed continuation capture" >&2
  exit 66
}

"$PYTHON_EXECUTABLE" "$RUNNER" --repo-root "$RUNTIME_REPO_ROOT" verify
"$PYTHON_EXECUTABLE" "$RUNNER" --repo-root "$RUNTIME_REPO_ROOT" \
  authorization-check --authorization "$AUTHORIZATION"

readarray -t CONFIG_PATHS < <(
  "$PYTHON_EXECUTABLE" - "$RUNTIME_REPO_ROOT" "$EXPERIMENT_ROOT" <<'PY'
import sys
from pathlib import Path

repo_root = Path(sys.argv[1]).resolve()
experiment_root = Path(sys.argv[2]).resolve()
sys.path.insert(0, str(experiment_root))
from pixie_etale_capture_v3.protocol import activate_source_imports, load_protocol
...
print(resolve_config_path(repo_root, config, "pixie_etale_motif_sharded_model_root").resolve())
print(resolve_config_path(repo_root, config, "pixie_etale_motif_output_root").resolve())
PY
)
[[ "$(realpath "${CONFIG_PATHS[0]}")" == "$SHARDED_MODEL_ROOT" ]] || {
  echo "launcher model root differs from the verified runtime config" >&2
  exit 66
}
[[ "$(realpath -m "${CONFIG_PATHS[1]}")" == "$OUTPUT_ROOT" ]] || {
  echo "launcher output root differs from the verified runtime config" >&2
  exit 66
}
...
ATTEMPT_DIRECTORY="$OUTPUT_ROOT/wrapper_v0_3/$ATTEMPT_ID"
[[ ! -e "$ATTEMPT_DIRECTORY" ]] || {
...
EXECUTION_SUMMARY="$ATTEMPT_DIRECTORY/capture_execution_summary.json"
...
MODEL_BLOCK_SOURCE="$(findmnt -rn -o SOURCE -T "$SHARDED_MODEL_ROOT" | tail -n 1)"
MODEL_DEVICE="$(findmnt -rn -o MAJ:MIN -T "$SHARDED_MODEL_ROOT" | tail -n 1 | tr -d '[:space:]')"
...
"$PYTHON_EXECUTABLE" "$RUNNER" --repo-root "$RUNTIME_REPO_ROOT" finalize \
