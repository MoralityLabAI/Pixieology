...
- `experiments/sheets_loops_levers_ui_v0_2/`: responsive thematic essay that carries one selected mechanism through an étale depth atlas, genuine closed-loop holonomy, registered-action reachability, and a coordinated synthesis claim; includes deterministic fixtures, agent-readable snapshots, contract tests, and a responsive browser receipt.
- `experiments/captain_rowan_tinylora_v0_1/`: exact-job continuation from the five-fact Captain Rowan prompt adapter to a rank-2 `gate_proj` TinyLoRA proposal, with 10 discovery and 10 frozen transfer rows, hash-bound continuation runner and Job Object wrapper, fail-closed authorization template, and no automatic model loading or training.
